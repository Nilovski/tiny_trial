...
This project wraps a 2x2 output-stationary integer systolic GEMM core
...
| `0x31` | READ_C_ELEMENT | 1 address byte (0-15)                       | 1 byte of C         |
| `0x40` | STATUS         | --                                            | `{7'b0, busy}`      |
| `0x41` | ID             | --                                            | `0xA2`              |

`C` is 4 accumulators x 4 bytes each (little-endian), address = `elem*4 +
byte_i`, `elem` in row-major order (c00, c01, c10, c11). There is no
accumulate-only mode yet -- `CLEAR_GEMM` always clears before multiplying.

## How to test

1. Send `0x41` (ID); expect `0xA2` back.
2. Send `0x10` then 4 bytes for A, then `0x11` then 4 bytes for B.
3. Send `0x21` to start the multiply.
4. Poll with `0x40` (STATUS) until bit 0 (busy) reads 0.
5. For each of the 16 result bytes, send `0x31` followed by the address
   byte (0-15); read back the corresponding byte of C.

A full example (LOAD_A / LOAD_B / CLEAR_GEMM / poll / READ_C_ELEMENT,
including negative two's-complement operands) is in
`test/test_gemm2x2.py`, verified against a Python reference multiply.
